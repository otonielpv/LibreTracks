...
const withImports = metadata.mode === 'import';
const groups = new Map();
for (const row of rows) {
  const key = [row.fill_threads, row.block, row.trim, row.preload, row.imports_requested ?? 0].join('/');
  if (!groups.has(key)) groups.set(key, []);
  groups.get(key).push(row);
}
if (groups.size !== (withImports ? 8 : 16)) throw new Error('Incomplete scenario set');
const median = values => {
...
const lines = [
  withImports ? '# Streaming con importación concurrente' : '# Streaming desde archivos: línea base', '',
  ...
  '| Fill workers | Buffer | Trim | Precarga | Preparación ms | p95 µs | p99 µs | Frames ausentes inicio / salto | Render tardío | Pico proceso MiB | CPU s |' + (withImports ? ' Importaciones | Import ms | Bloques con importación activa (suma) |' : ''),
  '| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |' + (withImports ? ' ---: | ---: | ---: |' : ''),
];
...
lines.push('', 'CPU s mide tiempo total del proceso durante la ventana, incluyendo lecturas y muestreo. ' +
  ...
  'interfaz ni validación térmica. El salto evita deliberadamente la espera de la capa de comandos de producción. ' +
