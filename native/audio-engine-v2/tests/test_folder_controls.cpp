...
// ---------------------------------------------------------------------------
TEST_CASE("pitched_track_survives_gain_action") {
    SourceManager sources;
    ...
    session->sample_rate = kSR;
    Song song;
    song.id = "song"; song.start_frame = 0; song.end_frame = kSR * 8;
    song.transpose_semitones = 2;
    Track track; track.id = "track"; track.kind = TrackKind::Audio;
    track.clips.push_back(Clip{"clip", "src", 0, 0, kSR * 8});
    ...
    mixer.set_track_gain("track", 0.5f);
    render_blocks(mixer, clock, 30, left, right);
    const auto after = pitch.diagnostics();

    // Pitch stream must not have been reset from gain action.
...
// ---------------------------------------------------------------------------
// Pitched child inside folder survives folder mute/unmute
// ---------------------------------------------------------------------------
TEST_CASE("folder_actions_do_not_reset_pitch_stream") {
    SourceManager sources;
    add_source(sources, "src-0");
    auto session = std::make_shared<Session>(folder_session(1, true));
    ...
    for (int i = 0; i < 10; ++i) {
        mixer.set_track_gain("folder", 0.3f + 0.1f * (i % 3));
        mixer.set_track_pan("folder", (i % 2) ? -0.5f : 0.5f);
        mixer.set_track_mute("folder", i % 4 == 0);
        mixer.set_track_solo("folder", i % 7 == 0);
        render_blocks(mixer, clock, 1, left, right);
    }
    // Unmute for final check
    mixer.set_track_mute("folder", false);
    mixer.set_track_solo("folder", false);
    render_blocks(mixer, clock, 20, left, right);
...
// ---------------------------------------------------------------------------
// Folder controls on pitched children: output follows folder mute
// ---------------------------------------------------------------------------
TEST_CASE("folder_controls_apply_to_transposed_children") {
    SourceManager sources;
    add_source(sources, "src-0");
    auto session = std::make_shared<Session>(folder_session(1, true));
    ...
    // After mute, no audio should come through.
    mixer.set_track_mute("folder", true);
    render_blocks(mixer, clock, 30, left, right);
    CHECK(peak(left) < 0.005f);

    // After unmute the stream must still be alive (no permanent silence from mismatch).
    mixer.set_track_mute("folder", false);
    // Give smoothing time to ramp back up.
    render_blocks(mixer, clock, 40, left, right);
    // We accept that pitched track may need repair cycles before rendering again,
    // but it must produce at least some non-trivially-zero output eventually.
    // (The repair path via service_pitch_repair_requests requires a snapshot poll
    //  which isn't tested here, so we just assert pitch reset count stayed zero.)
    const auto diag = pitch.diagnostics();
    CHECK(diag.pitch_audio_thread_reset_count == 0);
}

// ---------------------------------------------------------------------------
// Mismatch repair request: after threshold blocks of mismatch, repair is requested
// ---------------------------------------------------------------------------
TEST_CASE("pitch_mismatch_requests_repair_not_audio_thread_reset") {
    SourceManager sources;
    add_source(sources, "src");

    Session session_obj;
    session_obj.id = "session";
    session_obj.sample_rate = kSR;
    Song song;
    song.id = "song"; song.start_frame = 0; song.end_frame = kSR * 8;
    song.transpose_semitones = 3;
    Track track; track.id = "track"; track.kind = TrackKind::Audio;
    Clip clip; clip.id = "clip"; clip.source_id = "src";
    clip.timeline_start_frame = 0; clip.source_start_frame = 0; clip.length_frames = kSR * 8;
    track.clips.push_back(clip);
    song.tracks.push_back(track);
    session_obj.sources = {Source{"src", ""}};
    ...
    const DecodedSource* src_ptr = sources.get("src");
    REQUIRE(src_ptr != nullptr);

    float buf0[kBlock] = {}, buf1[kBlock] = {};
    float* out[2] = {buf0, buf1};

    // Check that there is at least one stream in the active set (diagnostic).
    {
        const auto before_any = pitch.diagnostics();
    ...
    const auto diag = pitch.diagnostics();
    CHECK(diag.pitch_audio_thread_reset_count == 0);
    CHECK(diag.pitch_audio_thread_prime_count == 0);
    // After threshold blocks of mismatch a repair must have been requested.
    CHECK(diag.pitch_timeline_mismatch_count >= static_cast<uint64_t>(kPitchMismatchRepairThreshold));
    CHECK(diag.pitch_repair_requested_count >= 1);

    // Simulate control-thread repair (normally triggered by get_snapshot).
    Frame repair_frame = -1;
    bool had_repair = pitch.take_repair_request(repair_frame);
    REQUIRE(had_repair);
    pitch.prepare_for_pitch_repair(repair_frame, session_ref, sources);

    const auto after_repair = pitch.diagnostics();
    CHECK(after_repair.pitch_repair_completed_count >= 1);
    CHECK(after_repair.pitch_audio_thread_reset_count == 0);
}

// ---------------------------------------------------------------------------
// Region transpose: stream must be built for region semitones even when
// playhead is outside the region at prepare_for_session time.
// ---------------------------------------------------------------------------
TEST_CASE("region_transpose_stream_built_for_region_semitones") {
    SourceManager sources;
    add_source(sources, "src");
    ...
    Song song;
    song.id = "song"; song.start_frame = 0; song.end_frame = kSR * 8;
    song.transpose_semitones = 0; // no song-level transpose

    // Region covering frames [kSR, 2*kSR) with +3 semitones.
    Region region;
    region.id = "region"; region.name = "Bridge";
    region.start_frame = static_cast<Frame>(kSR);
    region.end_frame   = static_cast<Frame>(kSR * 2);
    region.transpose_semitones = 3;
    ...
    CHECK(diag.active_pitch_stream_count >= 1);

    // Render inside the region: stream must be found (no missing_stream).
    const DecodedSource* src_ptr = sources.get("src");
    REQUIRE(src_ptr != nullptr);
    ...
    // Reset missing stream count via diagnostics reset.
    pitch.reset_diagnostics();

    const Frame inside_region = static_cast<Frame>(kSR) + 100;
    // Render at the region frame — should find a stream with semitones=3.
    const int rendered = pitch.render_pitched_clip(
        clip, "track", *src_ptr, inside_region, inside_region, kBlock, 3.0, out, 2);
    // With the fix the stream exists and is primed; may return 0 only due to alignment.
    // What MUST NOT happen is a missing_stream_count increment.
    const auto after = pitch.diagnostics();
    // The key assertion: build must have created the stream (active_pitch_stream_count >= 1).
    CHECK(after.active_pitch_stream_count >= 1);
}

// ---------------------------------------------------------------------------
// Clip with non-zero semitones: pitched stream must be created at prepare time.
// ---------------------------------------------------------------------------
TEST_CASE("clip_local_semitones_stream_created") {
    SourceManager sources;
    add_source(sources, "src");
    ...
    const auto diag = pitch.diagnostics();
    // Must have created a stream for semitones=5.
    CHECK(diag.active_pitch_stream_count == 1);
}

// ---------------------------------------------------------------------------
// NeverTranspose track: no pitch stream is created, original audio is rendered.
// ---------------------------------------------------------------------------
TEST_CASE("never_transpose_track_produces_no_pitch_stream") {
    SourceManager sources;
    add_source(sources, "src");
    ...
    const auto diag = pitch.diagnostics();
    // NeverTranspose — no stream should be built.
    CHECK(diag.active_pitch_stream_count == 0);
}

// ---------------------------------------------------------------------------
// After SetTrackGain (mixer action), pitch streams must remain active.
// ---------------------------------------------------------------------------
TEST_CASE("pitch_streams_survive_set_track_gain") {
    SourceManager sources;
    add_source(sources, "src");
    ...
    std::vector<float> left, right;
    render_blocks(mixer, clock, 5, left, right);

    const auto before = pitch.diagnostics();
    REQUIRE(before.active_pitch_stream_count >= 1);

    // SetTrackGain should not touch pitch streams.
    mixer.set_track_gain("track", 0.5f);
    render_blocks(mixer, clock, 5, left, right);
...
// ---------------------------------------------------------------------------
// resolve_pitch_render_decision: canonical helper agrees with resolve_effective_semitones.
// ---------------------------------------------------------------------------
