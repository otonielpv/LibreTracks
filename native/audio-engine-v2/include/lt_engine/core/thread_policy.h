...
enum class WorkerRole {
    ...
    // No hay Render aqui a proposito: los hilos del pool de render dependen de
    // la CLASE DE DISPOSITIVO, no de nucleos y RAM, asi que su politica vive en
    // device_profile.h (lt_recommend_render_threads_for). Tener la misma tabla
    // en dos sitios es como se rompen estas cosas.
};
