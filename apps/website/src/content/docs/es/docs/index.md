...
![Timeline de proyecto en LibreTracks](/screenshots/Proyecto.webp)
...
El modelo del proyecto es **song-first**: las canciones (regiones de cancion) son el contenedor principal, y los clips y pistas viven dentro de ellas. La app ofrece tres proyecciones equivalentes de ese modelo — el [timeline lineal (vista DAW)](/es/docs/core-concepts/) para arreglar, la [Vista Compacta](/es/docs/compact-view/) para ensayar e importar/exportar canciones, paquetes `.ltpkg` y puntos de partida desde proyectos externos, y la [Vista Live](/es/docs/live-view/) para el directo.
...
La reproduccion nunca se bloquea por la preparacion: al pulsar play el transporte arranca al instante, y cualquier pista cuyo audio aun se este decodificando permanece en silencio y entra sola en cuanto esta lista, asi las pistas ya preparadas nunca quedan retenidas por una fuente nueva lenta. Abrir una sesion tampoco espera: el timeline aparece de inmediato y el audio se sigue preparando en segundo plano con un aviso no modal mientras dura. Al abrir un proyecto LibreTracks tambien perfila la frecuencia de muestreo de su audio y, si el dispositivo lo admite, se alinea con ella para evitar convertir cada archivo; una frecuencia elegida a mano en `Configuracion` se respeta siempre.

La sesion abierta se guarda automaticamente cada cierto tiempo (por defecto cada 5 minutos, configurable en `Configuracion - General`), para que un fallo inesperado no se lleve el trabajo. El autoguardado nunca escribe mientras reproduces ni cuando no hay cambios pendientes.

![Guardado automatico de la sesion en Configuracion](/screenshots/Settings-General-Autosave.png)
...
- `Timeline (vista DAW)`: pistas de audio, carpetas y salidas MIDI; clips; regiones de cancion; transposicion por region; marcas; compases; edicion con rejilla; [senales de automatizacion](/es/docs/automation/); y organizacion por color. Los clips MIDI pueden enviar notas, controles, cambios de programa y curvas a hardware o software externo del show. Toda la interfaz se puede ampliar y ajustar a pantallas pequenas, y el timeline puede seguir al cursor de reproduccion.
- `Vista Live`: proyeccion para tocar del mismo modelo — cada marca de la cancion como un boton grande con su color, su cuenta atras y sus avisos, el repertorio con el progreso de cada cancion al lado, y los ajustes de salto de marca, salto de cancion y vamp siempre a mano. Se adapta desde una pantalla de escritorio hasta un movil, y no edita nada. Ver [Vista Live](/es/docs/live-view/).
...
- `Archivo`: volver a abrir sesiones recientes, crear desde `.lttemplate`, importar canciones/paquetes, importar proyectos Reaper/Ableton, importar o exportar una sesion entera como `.ltset` portable, guardar plantillas y exportar canciones preparadas. Ver [Integracion Y Ecosistema](/es/docs/integration-ecosystem/).

![Columnas de cancion en la Vista Compacta](/screenshots/Compact-View-Song-Columns.png)

![Marcas de seccion y de aviso en el Remote](/screenshots/Remote-Markers-Tablet.png)
