      ...
      deviceDescriptors: [
        {
          backend: "wasapi",
          backendId: "WASAPI",
          stableId: "WASAPI::Mock Built-in Output",
          name: "Mock Built-in Output",
          displayName: "WASAPI: Mock Built-in Output",
          isDefault: true,
          maxOutputChannels: 8,
          defaultSampleRate: 48000,
          supportedSampleRates: [44100, 48000],
          supportedBufferSizes: [128, 256, 512],
          supportedSampleFormats: ["f32"],
        },
      ],
    },
...
export const testDesktopApiMock = {
  ...
  pickAndImportExternalProjectIntoSession: async () =>
    clone({
      snapshot: buildSnapshot(),
      libraryAssets: state.libraryAssets,
    } satisfies SongPackageImportResponse),
  ...
  createClipsWithAutoTracks: async (
    args: Array<{ filePath: string; timelineStartSeconds: number }>,
  ) => {
    const nextTracks = [...state.song.tracks];
    const nextClips = [...state.song.clips];
    for (const entry of args) {
  ...
  moveClipToTrack: async (args: { clipId: string; targetTrackId: string }) => {
    ...
    const previousTrackId = sourceClip.trackId;
    const target = getTrack(args.targetTrackId);
    const nextClips = state.song.clips.map((clip) =>
      clip.id === args.clipId
        ? { ...clip, trackId: args.targetTrackId, trackName: target?.name ?? args.targetTrackId }
        : clip,
    );
    // Prune the previous track if it was auto-created and now has no clips.
    const previousTrack = getTrack(previousTrackId);
    const previousStillUsed = nextClips.some(
      (clip) => clip.trackId === previousTrackId,
    );
    const nextTracks =
      previousTrack?.autoCreated && !previousStillUsed
        ? state.song.tracks.filter((track) => track.id !== previousTrackId)
        : state.song.tracks;
    replaceSong({
      ...state.song,
      clips: nextClips,
