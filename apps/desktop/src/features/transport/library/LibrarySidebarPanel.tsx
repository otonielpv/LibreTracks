          ...
          return (
            <div key={asset.filePath} role="listitem" className={`lt-library-asset-row ${isSelected ? "is-selected" : ""}`}>
              ...
              >
                ...
                {/* Duration leads the row instead of trailing it: it is four
                    characters of tabular digits, while the name is the part
                    that actually tells assets apart and was being clipped to
                    "bass-1.w..." to keep a fixed right-hand column that only
                    matters WHILE importing. */}
                {isPending ? null : (
                ...
                {isPending ? (
                  <span
                    className="lt-library-asset-status"
                    // A failed import carries the backend's reason (e.g. "no
                    // cabe aqui: solo hay 0:12 libres antes de 'Song 5'").
                    // Surface it on hover — the status label alone only says
                    // "Error al importar", which reads as if the file were
                    // corrupt.
                    title={
                      asset.pendingError
                  ...
                  >
                    {getPendingClipLabel(asset.pendingStatus ?? "queued", t)}
                  </span>
                ) : null}
              </div>
            </div>
