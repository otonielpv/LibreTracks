...
/// Progress of an offline render, `fraction` in [0,1]. Return non-zero to keep
/// going, 0 to cancel. Mirrors `LtRenderProgressCallback` in lt_engine.h.
pub type LtRenderProgressCallback = unsafe extern "C" fn(ctx: *mut c_void, fraction: f64) -> i32;
...
extern "C" {
    ...
    pub fn lt_audio_engine_render_offline(
        request_json: *const c_char,
        on_progress: Option<LtRenderProgressCallback>,
        progress_ctx: *mut c_void,
    ) -> *const c_char;
...
pub unsafe fn lt_audio_engine_render_offline(
    _: *const c_char,
    _: Option<LtRenderProgressCallback>,
    _: *mut c_void,
) -> *const c_char {
    b"{\"ok\":false,\"error\":\"no-link\"}\0".as_ptr().cast()
}
