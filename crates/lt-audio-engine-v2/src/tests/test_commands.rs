...
fn seek_relative_negative_round_trip() {
    let cmd = EngineCommand::SeekRelative { delta_frames: -48000 };
    let rt = round_trip(&cmd);
    assert!(matches!(rt, EngineCommand::SeekRelative { delta_frames: -48000 }));
}
...
fn jump_to_marker_round_trip() {
    let cmd = EngineCommand::JumpToMarker { marker_id: "m1".into() };
    let rt = round_trip(&cmd);
...
fn jump_to_next_song_round_trip() {
    assert_eq!(round_trip_type(&EngineCommand::JumpToNextSong), "JumpToNextSong");
}
...
fn schedule_jump_immediate_round_trip() {
    let cmd = EngineCommand::ScheduleJump {
        jump_id: "j1".into(),
        target: JumpTarget { kind: JumpTargetKind::Marker, id: Some("m1".into()), frame: None },
...
fn cancel_scheduled_jump_round_trip() {
    let cmd = EngineCommand::CancelScheduledJump { jump_id: "j42".into() };
    let rt = round_trip(&cmd);
...
fn cancel_all_jumps_round_trip() {
    assert_eq!(round_trip_type(&EngineCommand::CancelAllScheduledJumps), "CancelAllScheduledJumps");
}
...
fn set_track_gain_round_trip() {
    let cmd = EngineCommand::SetTrackGain { track_id: "t1".into(), gain: 0.75 };
    let rt = round_trip(&cmd);
...
fn set_track_mute_true_round_trip() {
    let cmd = EngineCommand::SetTrackMute { track_id: "t1".into(), mute: true };
    let rt = round_trip(&cmd);
...
fn set_track_transpose_enabled_false_round_trip() {
    let cmd = EngineCommand::SetTrackTransposeEnabled { track_id: "t1".into(), enabled: false };
    let rt = round_trip(&cmd);
    assert!(matches!(rt, EngineCommand::SetTrackTransposeEnabled { enabled: false, .. }));
}
...
fn set_song_transpose_round_trip() {
    let cmd = EngineCommand::SetSongTranspose { song_id: "s1".into(), semitones: -3 };
    let rt = round_trip(&cmd);
    assert!(matches!(rt, EngineCommand::SetSongTranspose { semitones: -3, .. }));
}

#[test]
fn set_region_transpose_max_round_trip() {
    let cmd = EngineCommand::SetRegionTranspose { region_id: "r1".into(), semitones: 12 };
    let rt = round_trip(&cmd);
    assert!(matches!(rt, EngineCommand::SetRegionTranspose { semitones: 12, .. }));
}
...
fn set_output_device_round_trip() {
    let cmd = EngineCommand::SetOutputDevice { device_id: "Speakers (Realtek)".into() };
    let rt = round_trip(&cmd);
    assert!(matches!(rt, EngineCommand::SetOutputDevice { device_id } if device_id.contains("Realtek")));
}
...
fn set_sample_rate_round_trip() {
    ...
    let rt = round_trip(&cmd);
    assert!(matches!(rt, EngineCommand::SetSampleRate { sample_rate: 44100 }));
}
...
fn set_buffer_size_round_trip() {
    ...
    let rt = round_trip(&cmd);
    assert!(matches!(rt, EngineCommand::SetBufferSize { buffer_size: 256 }));
}
...
fn load_session_round_trip() {
    let cmd = EngineCommand::LoadSession { project_json: r#"{"id":"s1"}"#.into() };
    let json = serde_json::to_string(&cmd).unwrap();
