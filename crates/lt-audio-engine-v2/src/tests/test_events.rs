...
fn playback_started_type() {
    assert_eq!(
        type_field(&EngineEvent::PlaybackStarted { frame: 0 }),
        "PlaybackStarted"
    );
}
...
fn playback_stopped_type() {
    assert_eq!(
        type_field(&EngineEvent::PlaybackStopped { frame: 0 }),
        "PlaybackStopped"
    );
}
...
fn seek_executed_preserves_frames() {
    let ev = round_trip(EngineEvent::SeekExecuted {
        from_frame: 1000,
        to_frame: 48000,
    });
    assert!(matches!(
        ev,
        EngineEvent::SeekExecuted {
            from_frame: 1000,
            to_frame: 48000
        }
    ));
...
fn jump_cancelled_preserves_id() {
    let ev = round_trip(EngineEvent::JumpCancelled {
        jump_id: "j42".into(),
    });
...
fn device_error_preserves_message() {
    let ev = round_trip(EngineEvent::DeviceError {
...
fn source_prepared_type() {
    assert_eq!(
        type_field(&EngineEvent::SourcePrepared {
            source_id: "s1".into()
        }),
        "SourcePrepared"
    );
...
fn source_starved_type() {
    let ev = EngineEvent::SourceStarved {
        source_id: "s1".into(),
        track_id: "t1".into(),
    };
...
fn pitch_cache_prepared_preserves_semitones() {
    let ev = round_trip(EngineEvent::PitchCachePrepared {
        source_id: "s1".into(),
        semitones: -5,
    });
    assert!(matches!(
        ev,
        EngineEvent::PitchCachePrepared { semitones: -5, .. }
    ));
}

#[test]
fn diagnostic_warning_preserves_message() {
    let ev = round_trip(EngineEvent::DiagnosticWarning {
...
fn all_events_have_type_field() {
    let events: &[EngineEvent] = &[
        ...
        EngineEvent::SourcePrepared {
            source_id: "".into(),
        },
        EngineEvent::SourceStarved {
            source_id: "".into(),
            track_id: "".into(),
        },
        EngineEvent::PitchCachePrepared {
            source_id: "".into(),
            semitones: 0,
        },
